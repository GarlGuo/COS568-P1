...
def run(args):
    ...
    print(f"Inference time on testing dataset, post-pruning: {post_result['inference_time'].iloc[-1].item():.3f} sec ")
    ...
    print("Train results:\n", train_result)
    print("First 20 lines of prune results:\n", prune_result.head(20))
    print("Parameter Sparsity: {}/{} ({:.4f})".format(total_params, possible_params, total_params / possible_params))
    print("FLOP Sparsity: {}/{} ({:.4f})".format(total_flops, possible_flops, total_flops / possible_flops))

    if args.save_trace:
        print('Save PyTorch trace for 1 iteration on testing dataset.')
